...
public class TestSearch {
    ...
    //MatchQuery 先分词后搜索 一次只能查一个域  还可以设置匹配度
    // 分词个数 * 匹配度  向下取整数  即至少匹配的词的个数
    @Test
    public void testMatchQuery() throws IOException, ParseException {
        ...
        //搜索方式
        //MatchQuery   minimumShouldMatch是设置的匹配度
        //spring  开放  框架  分了3个词  匹配度是0.8  3*0.8=2.4   向下取整  即至少需要匹配2个词
    ...
    //MultiMatchQuery  可以查询多个域  并且可以设置权重boost
    @Test
    public void testMultiMatchQuery() throws IOException, ParseException {
        ...
        //搜索方式
        //MultiMatchQuery   name和description是两个域
        //.field("name",10)); -》将name的权重增加10倍
    ...
    //filter  过滤器  只针对结果过滤 不对字段进行评分  所以效率很高
    @Test
    public void testFilter() throws IOException, ParseException {
        ...
        BoolQueryBuilder boolQueryBuilder = QueryBuilders.boolQuery();
        boolQueryBuilder.must(multiMatchQueryBuilder);
        //定义过虑器  过滤出结果 字段studymodel 必须是 201001的结果
